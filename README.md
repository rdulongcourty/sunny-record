...
- **Factures** : nouvel onglet Gestion → Factures. Remplissez le client, la date, les lignes de commande (description/quantité/prix) et le taux de TVA — un PDF est généré et téléchargé instantanément (numéro de facture attribué automatiquement, format `FA-2026-0001`). Chaque facture reste dans un historique consultable, avec statut « En attente »/« Payée » modifiable et un bouton pour retélécharger le PDF à tout moment.

  ⚠️ Ce générateur ne connaît pas votre situation légale (SIRET, régime de TVA, mentions obligatoires) — utilisez le champ « Notes » pour les ajouter vous-même, ou vérifiez avec votre comptable ce qui est requis pour vos factures.
