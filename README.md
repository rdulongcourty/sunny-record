...
- **Likes sur les morceaux** : sur la page Artistes, un cœur cliquable à côté de chaque nom affiche et incrémente un compteur de likes — aucun compte visiteur requis. Le navigateur retient ce que la personne a déjà aimé (pour afficher un cœur plein et permettre de retirer son like), stocké localement dans son navigateur. Le nombre de likes de chaque artiste est aussi visible dans Gestion → Artistes.
- **Compteur d'écoutes** : à côté du like, un petit indicateur 🎧 compte le nombre de fois où le morceau de chaque artiste a été lancé.
- **Concerts / événements** : nouvelle page publique `concerts.html`, gérée depuis Gestion → onglet Concerts (titre, date, heure, lieu, ville, lien billetterie, description). Les concerts à venir s'affichent en premier, les concerts passés apparaissent ensuite dans une section "Archives".
- **Factures** : nouvel onglet Gestion → Factures. Remplissez le client, la date, les lignes de commande (description/quantité/prix) et une réduction optionnelle en % (0 par défaut) — un PDF est généré et téléchargé instantanément (numéro de facture attribué automatiquement, format `FA-2026-0001`). Chaque facture reste dans un historique consultable, avec statut « En attente »/« Payée » modifiable et un bouton pour retélécharger le PDF à tout moment.

  **Lien avec la Comptabilité** : passer une facture en « Payée » l'ajoute automatiquement comme entrée d'argent dans l'onglet Comptabilité (catégorie "Facture", montant = total de la facture). Repasser en « En attente » retire cette même entrée — pas de double comptage possible en basculant plusieurs fois.

  Ce générateur ne connaît pas votre situation légale (SIRET, régime de TVA, mentions obligatoires) — utilisez le champ « Notes » si vous voulez en ajouter.
